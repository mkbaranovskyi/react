...
- **Controlled** component's inner value is set by React (via `setState`). We can defer applying a value to a component and have time to inspect it, log it, etc. Also we can easily access the value of your component as it's always stored in a State.
- **Uncontrolled** component's inner value is set by user (e.g. `<input type="file" />`).
...
const ControlledInput: React.FC = () => {
	...
	const onChange: React.ChangeEventHandler<HTMLInputElement> = (e) => {
		...
		// Comment the next line and you'll see how you component stopped working - the user input doesn't mean anything as WE are the ones controlling the component via state
