...
You can also **return a function** from `useEffect` - this is an optional cleaning up mechanism which runs **right before** the next `useEffect` (or before the unmounting).
