...
Basically a wrapper function that adds new functionality to your components.

HOC doesn't modify the original components - it's a pure function that only returns a new component
...
Imagine we had these components:
...
The two components are nearly identical. We could have even more components having the same functionality. Yet, all of them have to implement almost the same functionality separetely. we need an **abstraction** to optimize our code and **share** functionality with any number of components.
