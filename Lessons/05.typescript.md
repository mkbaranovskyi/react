...
You can get this error if you use an incorrect event type. E.g. simple controlled `<input type="text" />` produces `FormEvent`, not `KeyboardEvent`:
...
Fix is to assure TS that `key` is always a `string`:
