...
4. `const ref = useRef<HTMLInputElement>(null)` - refs (also don't miss the default value)
5. `const [title, setTitle] = useState<string>('')` - state
6. `ref.current!.value = ''` - the exclamation mark `!` ensures TS that the object will have the `value` field.
